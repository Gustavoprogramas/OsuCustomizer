...
namespace OsuCustomizer
{
    public partial class Form1 : Form
    {
        ...
        private void UpdateUIFromSettings()
        {
            ...
            if (txtFPS != null)
            {
                txtFPS.Text = _currentSettings.target_fps.ToString();
            }
        ...
        private void btnMenuBlur_Click_2(object sender, EventArgs e)
        {
            MostrarPainel(pnlBlur);
        }

        private void btnMenuStable_Click_1(object sender, EventArgs e)
        {
            MostrarPainel(pnlStable);
        }

        private void btnPanelLzr_Click_1(object sender, EventArgs e)
        {
            MostrarPainel(pnlLazerDX11);
        }

        private void btnAplicarFPS_Click(object sender, EventArgs e)
        {
            
            if (int.TryParse(txtFPS.Text, out int fpsEscolhido))
            {
                
                if (fpsEscolhido < 0) fpsEscolhido = 0;

               
                _currentSettings.target_fps = fpsEscolhido;
                UpdateSharedMemory();

                
                if (fpsEscolhido == 0)
                {
                    MessageBox.Show("Desativado com Sucesso", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show($"Limitado em {fpsEscolhido}Fps", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
               
                MessageBox.Show("Para de ser autista", "Entrada Inválida", MessageBoxButtons.OK, MessageBoxIcon.Warning);
